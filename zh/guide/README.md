...
## 配置说明
   ...
   > 首先创建一个市场的基础数据表
      数据以散列hash表方式存储，主要有以下几种key值；
      
      市场号 -- 储存该市场的基础信息和配置，比如开收市时间，时区，市场名称等，这些信息以json格式存储在info字段中；
      市场号+股票代码 -- 存储一个股票的信息，携带一个json格式的info字段，保存该股票的特定信息；

      info中默认且必须的字段配置：
      1、price-unit   -- 价格单位   默认为 100
         定义为 price 的字段必须对传入的值放大后取整写入，输出时再缩小以 float 输出；

      2、volume-unit -- 成交量单位  默认为 100
      ...
      7、trade-date -- 当前交易日期 -> 市场专用

      8、close-date -- 收市日期 -> 市场专用

      9、status  --  状态，0 初始化等待 1 正常运行 3 收市
      
      10、 trade-time --交易时间段  [[930,1130],[1300,1500]]

      11、 init-time -- 初始化时间  900
   ...
   > subscribe-method: 
      ...
      1、 copying  特指 vol 和 money 直接原样复制；
      2、 increase 特指 vol 和 money 和前一时间数据的增量；
